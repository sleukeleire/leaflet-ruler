(function(factory, window){
  ...
  L.Control.Ruler = L.Control.extend({
      ...
      _toggleMeasure: function() {
          ...
          this._clickedPoints = [];
          this._totalLength = [];
          for (var i = 0; i < this.options.lengthUnit.length; i++) {
      ...
      _renderDistances: function(distance) {
          ...
          for (var i = 0; i < this.options.lengthUnit.length; i++) {
              ...
              text += distance[i].toFixed(this.options.lengthUnit[i].decimal) + '&nbsp;' + this.options.lengthUnit[i].display
          }
          return text;
      },
      _escape: function(e) {
      ...
      _calculateBearingAndDistance: function() {
          ...
          var distance = [];
          for (var i = 0; i < this.options.lengthUnit.length; i++) {
              distance[i] = c * (this.options.lengthUnit[i].factor ? 6371 * this.options.lengthUnit[i].factor : 6371); // default is kilometres
          }
          this._result = {
              Bearing: brng,
              Distance: distance
